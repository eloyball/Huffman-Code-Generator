...
std::string HuffmanTree::compress(const std::string inputStr)
{
    ...
    tq.insert(new HuffmanNode('\0', 0));
    while(!hq.empty()) {
        tq.insert(hq.min());
        std::cout << hq.min()->getCharacter() << std::endl;
        hq.removeMin();
    }
    tq.removeMin();
    ...
    fq.insert(new HuffmanNode('\0', 0));
    while(!tq.empty()) {
        fq.insert(tq.min());
        std::cout << tq.min()->getCharacter() << std::endl;
        tq.removeMin();
    }
    fq.removeMin();
    ...
    char pChar = '\0';
    while(fq.size() > 1){
        HuffmanNode *hnL = fq.min();
        fq.removeMin();

        HuffmanNode *hnR = fq.min();
        fq.removeMin();
